...
#[cfg(feature = "render")]
use serval_layout::{Applied, IncrementalLayout};
use serval_scripted_dom::{NodeId, ScriptedDom};

fn capture_dir() -> Option<&'static PathBuf> {
    static DIR: OnceLock<Option<PathBuf>> = OnceLock::new();
    DIR.get_or_init(|| std::env::var_os("SERVAL_DOM_CAPTURE_DIR").map(PathBuf::from))
        .as_ref()
}

fn capture_viewport_seed() -> io::Result<(u32, u32)> {
    Ok((
        capture_dimension("SERVAL_DOM_CAPTURE_WIDTH", 1280)?,
        capture_dimension("SERVAL_DOM_CAPTURE_HEIGHT", 720)?,
    ))
}
...
pub(crate) fn replay_capture(path: &Path) -> io::Result<ReplayReport> {
    ...
    for record in records.iter().skip(1) {
        ...
        if layout.is_some() {
            return Err(io::Error::new(
                io::ErrorKind::InvalidData,
                "capture file includes layout parity data but serval-scripted was built without render",
            ));
        }
        batch_count += 1;
    }
...
#[cfg(feature = "render")]
fn fragment_digest(dom: &ScriptedDom, fragments: &serval_layout::FragmentPlane<NodeId>) -> u64 {
...
mod tests {
    ...
    fn temp_capture_path() -> PathBuf {
        ...
        std::env::temp_dir().join(format!("serval-dom-capture-{}-{unique}.bin", now_millis()))
    }
