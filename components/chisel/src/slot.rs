...
impl SceneSlot {
    pub fn new(intrinsic: Size) -> Self {
        Self { pending: None, intrinsic, dirty: false }
    }
...
mod tests {
    ...
    fn scene_slot_splices_external_texture_and_gates_epochs() {
        ...
        assert!(out.scenes().next().is_none(), "no scene until one is pushed");
        ...
        reg.get_mut_as::<SceneSlot>(&77).unwrap().set_scene(a_scene());
        assert_eq!(reg.render_into(sized, &mut out), 1);
        ...
        let bigger = |_k: u64| Some(Size { width: 64.0, height: 40.0 });
