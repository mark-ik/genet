...
//! `genet-layout`-owned planes (`StylePlane`) keyed by `D::NodeId`, read via
//! plane accessors during the cascade rather than embedded on the handle.
//!
//! See `docs/2026-05-17_genet_layout_planes_architecture.md` for the
...
/// atomized id/class) lives in `genet-layout`-owned planes (`StylePlane`)
