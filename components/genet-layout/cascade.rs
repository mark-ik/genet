...
//! architecture where style state lives in `genet-layout`-owned planes
...
/// preference + capability feature values genet's `mark-ik/stylo` fork exposes
    ...
    D::NodeId: Copy + Eq + Hash + 'static,
{
    ...
    // GENET_LAYOUT_TIMING to split the cascade cost into its sub-phases —
    ...
    // `2026-06-19_cross_platform_parallelism_strategy.md` §0 and genet
...
/// genet has its own, more complete layout, so the gate is servo's policy, not
/// genet's. Enabling lets the cascade *parse* them (`text-overflow`,
/// `user-select`, `backdrop-filter`, `contain`, counters, `mask-image`, `zoom`,
/// …); genet only changes rendering where it actually reads one
/// (`text-overflow` today), so the rest are computed-but-unused until genet
/// grows support. Set once — the pref store is a process-global shared with
/// Stylo, and the parse-time check reads it.
fn enable_genet_properties() {
    use std::sync::Once;
    ...
    ONCE.call_once(|| {
        ...
        // off by default because *servo's* layout never implemented grid. genet
        // dispatches `display: grid` to Taffy's grid algorithm (`box_tree`), so
        // the gate is servo's policy, not genet's — enable it so the cascade
    ...
    D::NodeId: Copy + Eq + Hash + 'static,
{
    ...
    // are paint-tier for genet's taffy-driven layout.
...
/// traversal, which genet does not run.
///
...
mod tests {
    ...
    /// `:root` matches the document's root element. genet's root element
    ...
    /// The `prefers-reduced-motion` media feature (genet's `mark-ik/stylo`
