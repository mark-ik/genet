...
//!
//! It reads the `parley::Layout` genet already caches per inline-formatting leaf
...
/// hit). The HTML/genet lane has no host-queryable packet, so the content actor runs
...
/// wrapper over the canonical [`genet_lane::absolute_origin`](crate::genet_lane::absolute_origin)
...
where
    ...
    D::NodeId: Copy + Eq + Hash,
{
    crate::genet_lane::absolute_origin(dom, fragments, target).map(|p| (p.x, p.y))
}
