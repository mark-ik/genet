...
//!
//! Splits genet-layout's cold cost into cascade / box-tree-build / shaping /
...
//! cargo run -p genet-layout --release --example phase_timing
...
//! GENET_LAYOUT_TIMING=1 cargo run -p genet-layout --release --example phase_timing -- --iters=2 path/to/page.html
//! ```
//!
...
fn main() {
    ...
    // When GENET_LAYOUT_TIMING is set, layout_via_box_tree prints its four
    // sub-phases per pass; mark the passes so the warmest (last) is identifiable.
    let lib_timing = std::env::var_os("GENET_LAYOUT_TIMING").is_some();
    ...
    if lib_timing {
        ...
        println!("(re-run with GENET_LAYOUT_TIMING=1 --iters=2 for the layout phase split)");
    }
}
