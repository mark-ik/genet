...
//!
//! The meerkat chrome shell and the orrery host are both "a genet surface
...
impl RenderCore {
    ...
    fn scene_target(&self, w: u32, h: u32) -> (wgpu::Texture, wgpu::TextureView) {
        ...
        let tex = device.create_texture(&wgpu::TextureDescriptor {
            label: Some("genet-winit-host scene"),
            size: wgpu::Extent3d {
                width: w,
                height: h,
        ...
        let view = tex.create_view(&wgpu::TextureViewDescriptor {
            label: Some("genet-winit-host scene view"),
...
impl WindowSurface {
    ...
    pub fn acquire(&self, core: &RenderCore) -> Option<wgpu::SurfaceTexture> {
        match self.surface.get_current_texture() {
            ...
            other => {
                eprintln!("[genet-winit-host] surface acquire skipped: {other:?}");
                None
            },
        }
    }
