...
//! genet's mutable [`ScriptedDom`](genet_scripted_dom::ScriptedDom).
//!
//! This is the third `xilem_core` backend, beside `xilem` (native, → Masonry)
//! and `xilem_web` (→ the browser DOM via `web_sys`), pointed at genet's
//! [`LayoutDomMut`](layout_dom_api::LayoutDomMut). It is `xilem_web`, but
//! native, with genet as the engine: app state → view tree → diff → DOM
//! mutations, doing no layout, paint, or hit-testing.
//!
//! # The key simplification
//!
//! `xilem_web` carries `Box<dyn AnyNode>` because the browser exposes several
//! distinct node Rust types. In genet every DOM node is one type
//! ([`NodeId`](genet_scripted_dom::NodeId)), so the backend has **no type
...
//! `set_attribute` records a `DomMutation`); genet batches at the
//! `drain_mutations` → relayout boundary, so the deferred-apply-on-`Mut`-drop
//! machinery from `xilem_web` is dropped entirely.
//!
//! # Status
//!
//! Stage 3a of `docs/2026-05-27_genet_as_host_xilem_serval_plan.md`: the
//! backend probe (Stage 1a) plus [`GenetAppRunner`], the genet-native owner
...
pub use keyed::Keyed;
pub use multi::{ProjectionId, GenetMultiRunner};
...
// `GenetCtx` with no genet-side impl; re-exported here so chrome authors can
...
/// The HTML namespace. genet views build elements in this namespace, matching
/// `xilem_web`'s `HTML_NS`.
pub const HTML_NS: &str = "http://www.w3.org/1999/xhtml";

/// A shared, mutable handle to the document every view in a tree mutates.
///
/// A genet-native runner (Stage 1b's `GenetAppRunner`) will own scheduling
