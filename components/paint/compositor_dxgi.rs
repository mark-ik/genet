...
/// Per-`SurfaceKey` DCOMP node. Holds the visual + the destination
/// texture the master blits into.
struct DxgiSurface {
...
impl WindowsDxgiBackend {
    ...
    pub fn present_master(&mut self, master: &Texture) -> Result<(), BackendError> {
        ...
        unsafe {
            ...
            let lists: [Option<ID3D12CommandList>; 1] = [Some(self.cmd_list.cast().map_err(
                |e| BackendError::DComp(format!("CommandList cast: {e}")),
            )?)];
...
impl OsCompositorBackend for WindowsDxgiBackend {
    ...
    fn destroy(&mut self, key: SurfaceKey) {
        // The visual drops when the entry is removed; DCOMP holds its
        // own ref via the visual tree but Commit on the next frame
        // will reflect the removal once SetRoot/AddVisual logic lands.
...
impl std::fmt::Display for BackendError {
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        match self {
            ...
            Self::NoHalDevice => f.write_str("WindowsDxgiBackend: wgpu-hal Dx12 device unavailable"),
