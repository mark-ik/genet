...
**Date:** 2026-06-24
**Status:** E1 landed 2026-06-30. E2 landed + verified 2026-06-30. E3 event-loop/task-source surface landed 2026-06-30. E4 trace capture + TLA witness wired 2026-06-30; broader protocol specs still open. Spun out of the gterzian/formal-web harvest (`2026-06-24_formal_web_lessons.md`, ideas 3/4/7 + the two bug-rules) and the grand audit §6 ("the gaps are granularity, not architecture").
...
- 2026-06-30 — **E4 TLA witness wired.** Added `docs/tla/scheduler_trace/SchedulerTrace.tla` + `SchedulerTrace.cfg`, a checked-in generated `SchedulerTraceData.tla` from the fixture NDJSON, `components/script-runtime-api/tools/scheduler_trace_to_tla.py`, and `support/ci/check_scheduler_trace_tla.py`. CI now downloads `tla2tools.jar` and runs TLC over the fixture model before the workspace build. Local verification covered generator/data drift; TLC itself was not run locally because Java is not installed in this Windows environment.
