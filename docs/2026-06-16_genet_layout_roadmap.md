# genet-layout: state and roadmap

**Date:** 2026-06-16. **Scope:** the `genet-layout` crate (the engine that
...
HTML/genet lane). The planes model from
`2026-05-17_genet_layout_planes_architecture.md` is realized: DOM / Style /
...
traits directly against genet's own box tree; the old `cv_to_taffy` oracle is
retired and parity is held by inline tests rather than a shadow tree. Paint
emits a `GenetPaintList` (`paint_emit.rs`); inline formatting, replaced boxes
...
- `2026-06-02_genet_holistic_audit.md` — cross-subsystem state.
- `2026-06-07_genet_layout_infrastructure_scope.md` — interaction state,
...
- `2026-05-17_genet_layout_planes_architecture.md` — the planes model.
...
- **Find-in-page primitive** (`caret::find_text_rects`,
  ...
  (committed genet `28e3e55`, mere `eefbed6`).
...
genet-layout's seat the engine-side primitive (the external-texture replaced
box and its paint pass) is done; the scripted tier's layout coupling routes
through `IncrementalLayout` + `genet_scripted::relayout_if_dirty`, and pelt V4
...
scripted-DOM breadth, not a genet-layout engine primitive.
...
- **`position: fixed` resolves its insets against the *parent*, not the viewport**
  — **promoted to `2026-07-11_position_containing_block_plan.md`; the plan is
  COMPLETE** (2026-07-11/12): F1 (fixed -> ICB), F2 (absolute -> nearest
  positioned ancestor), the hit-walk/predicate residual round, out-of-flow
  islands in inline content, static-position machinery (placeholder +
  fixup), sticky V1 (document scrollport), synthetic multi-root ICB sizing,
  positioned inline-block CBs (gated on pure-inline content), and
  row-relative table offsets. The WPT wheel quartet went green (cluster
  42/42); css-position reftests 60 -> 62. One big rock remains, named there:
  inline-blocks with block-level content need real box subtrees. The entry
  ...
  (the ICB); genet used the Taffy parent. Probe: for
  ...
  **The scope is larger than "fix a mapping", and this is the part to know before
  starting.** genet-layout has *no* fixed-vs-absolute handling at all: it hands
...
3. `2026-05-17_genet_layout_planes_architecture.md` (the model).
