...
README, workspace `Cargo.toml`), mapped against genet + mere seams (file:line).
...
Rendering is not pixel-perfect yet; overall fidelity is years behind genet's
...
1. **It closes the exact deferrals the first cut names.** genet-layout's
   ...
   generation per §17.2.1 (genet currently drops stray content,
...
3. **The integration shape matches genet-layout exactly and keeps the taffy
   fork clean.** Host-owned tree + `layout_cell` recursion + `set_layout`
   writeback means genet-layout keeps owning the box tree, taffy keeps laying
   out cell content, and the table algorithm becomes a leaf module inside
   genet-layout instead of a taffy-fork feature (no new fork skew). The seam
...
4. **Test furniture comes with it.** Lattice ships `src/mock.rs`, a fixture
   ...
   genet-layout's existing hand-computable integer-px test style
...
which is fine for one document at a time. The lesson activates when a genet
...
- the genet-side DOM-storage seam already exists: the `StorageProvider` host
  hook (`script-runtime-api/platform.rs:119`, genet `3ed0ed0`, tested on boa
  ...
  webstorage_thread.rs`; only relevant if genet-scripted becomes a live
  meerkat lane.)
...
- `gosub_webinterop` / `gosub_webexecutor`: proc-macro JS-binding codegen over
  ...
  table genet already shipped (`archive/2026-06-24_html_interface_table_plan.md`),
...
- parser fuzzing (`gosub_html5`/`gosub_css3` fuzz dirs): transfers to nematic's
  smolweb parsers only; genet's html5ever/stylo lanes are upstream-tested.
...
- `gosub_taffy`: genet's taffy fork is deeper than their integration.
...
- The zones/tabs embedder API itself (`TabCommand`/`EngineEvent`): genet and
...
- **Tables:** a scoped genet-layout plan of its own when picked up; the
