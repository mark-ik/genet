...
This document is the authority for Genet's in-browser script-engine selection.
...
supports wasm64 and Genet locks 0.2.126. Rust's `wasm64-unknown-unknown` target is
...
- `script-engine-nova` and `genet-scripted` gate Nova on
  `target_pointer_width = "64"`, admitting native64 and wasm64 while rejecting
  wasm32.
- The Genet Nova fork gates USDT off `target_family = "wasm"`, supplies the same
...
- `ScriptedDocument<E>` is public from `genet-scripted`; Pelt re-exports and
...
- `genet-scripted-worker` is backend-exclusive. `engine-nova` and `engine-boa`
...
- `support/build-scripted-workers.ps1` pins nightly `nightly-2026-06-22`, requires
  ...
  - `genet-scripted-nova-wasm64`
  - `genet-scripted-boa-wasm32`
...
node --test components/genet-scripted-worker/loader.test.mjs
cargo test -p genet-scripted-worker
cargo test -p genet-scripted-worker --no-default-features --features engine-nova
```
...
`genet-scripted` suite remains the shared backend-neutral fixture set for event
