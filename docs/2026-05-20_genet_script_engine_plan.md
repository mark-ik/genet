# genet script engine plan — modular JS backends, prerender lane, WPT trajectory

**Status (2026-05-20):** proposed; for review. Designs the scripting layer that the
[profile-ladder plan](./2026-05-12_genet_profile_ladder_plan.md) left as the
`genet-scripted` tier's open interior, after the 2026-05-15 SpiderMonkey
...
(primary) and Boa (oracle) backends, `genet-scripted-dom` + the reflector bridge, and
...
records; genet-layout (not the DOM provider) translates them into
...
reframed as a Genet **tier-selection escalation**, not a Hekate E3 extraction
...
stays a *free variable* rather than re-marrying Genet to one engine.

Sibling reads:

- [2026-05-12 profile-ladder plan](./2026-05-12_genet_profile_ladder_plan.md) — the static/interactive/scripted/fullweb tier framing; this plan is the scripted interior.
...
The 2026-05-15 cut already paid down the hard prerequisite. `genet-static-dom`
...
not the engine abstraction. The relationship is: a future `genet-scripted`
...
| --- | --- | --- | --- | --- | --- |
| **static** | `genet-static-html` / `-interactive-html` | live Rust DOM, no JS | one (Rust) | none | none (mozjs-free, audit canary) |
...
| **live scripted** | `genet-scripted` | real Genet DOM mutated through bindings; layout reacts | two (the marriage) | full `LayoutDomMut` bridge | any |
| **fullweb** | `genet-fullweb` | live + workers/storage/media/etc. | two + service heaps | full + platform APIs | near-100%-conformant VM |
...
A wasm32 build of `genet-scripted` depends on the quickjs/boa/nova impls and
...
`genet-static-html` / `-interactive-html` pull *no* `script-engine-*` crate at
all; the wasm `genet-scripted` graph contains no `mozjs`.
...
refcount+CC). Three deltas for Genet:
...
New crate `genet-prerender`, depending on `script-engine-api` (+ one backend) and
`genet-static-dom`. **Not** on `layout-dom`'s mutation surface — it doesn't have
one.
...
```text
HTML bytes
  → html5ever parse → extract <script> sources in document order  (genet-static-dom already parses; reuse its sink)
  ...
  → hand to genet-layout planes → Paint  (the normal static path)
```
...
thread-locals; the readback feeds `genet-static-dom` rather than a CLI string.

### Sandbox & capability boundary (mandatory for remote content)

Prerender runs **arbitrary, often hostile, remote JS** — and Graphshell/Mere/Genet
...
- **`fetch`/XHR in prerender is opt-in and brokered.** rakers' `_r_fetch_sync`
  does live synchronous GETs by default; for Genet that capability is off unless
...
Prerender is a **Genet tier-selection escalation**, not a Hekate extraction tier.
...
the static/prerender/scripted/fullweb tiers are *render* selections Genet owns.
...
handing the document to the Genet lane; **Genet** decides to instantiate the
...
- `genet-static-dom` / reader-mode / serialization / selector-matching consume
...
- The scripted-DOM provider (new `genet-scripted-dom`, parallel to
  `genet-static-dom`) implements **both** `LayoutDom` and `LayoutDomMut`.
- This is where the GC marriage is real and per-backend: the `genet-scripted-dom`
...
**Invalidation is genet-layout's job, not the DOM's.** The DOM provider emits
...
belongs to genet-layout. Instead, genet-layout's scheduler **consumes** the
...
on the genet-layout side — on the `StyleElement` adapter and its `NodeId`-keyed
...
layout_dom_api doc's deferred "P4 problem," now owned by genet-layout's scheduler.
...
1. **Engine conformance** — does the VM implement ECMAScript correctly?
   Measured by **test262**, *fixed per backend*, not Genet's work:
...
2. **Binding conformance** — does Genet expose the DOM/HTML/CSSOM platform
   correctly? Measured by **WPT** (`dom/`, `html/`, `cssom/`, `css/...`),
   *entirely Genet's work*, engine-independent above the language layer.
...
`genet-scripted` will cost *some* DOM-test passes to language gaps — but expect
...
| --- | --- | --- |
| `css/CSS2/`, `css/css-flexbox/` reftests | `genet-static-html` (+a reftest runner that doesn't need testharness.js) | Stylo+Taffy (see [stylo_taffy plan](./2026-05-20_stylo_taffy_adoption_plan.md)) |
| `dom/nodes/`, `dom/events/` (testharness) | `genet-scripted` (live) | `ScriptEngine` + `LayoutDomMut` bindings |
| `html/semantics/` DOM APIs | `genet-scripted` (live) | bindings breadth |
| `html/dom/` reflection, `cssom/` | `genet-scripted` (live) | bindings breadth |
| `service-workers/`, `workers/`, `fetch/`, `streams/`, `storage/` | `genet-fullweb` | platform APIs + constellation |
| `webgpu/`, `webgl/` | `genet-fullweb` | canvas providers |

Two runner shapes follow:

- **Reftest runner** — renders two pages, compares pixels. Needs no JS for pure
  CSS reftests. Lives at the static tier; the first WPT signal Genet can produce
...
- **T-scripted-core:** testharness runner boots `genet-scripted`; `dom/nodes/`
...
## Part 5 — the modular pattern, generalized to genet's other big parts
...
Applied to Genet's big parts:

| Subsystem | Contract crate | Providers | Witness / status |
| --- | --- | --- | --- |
| **DOM** | `layout-dom-api` (exists) | `genet-static-dom` (exists), `genet-scripted-dom` (this plan) | static profile = static-dom only; scripted adds `LayoutDomMut` impl |
...
| **Layout** | `layout-api` + the planes (exists; [planes doc](./2026-05-17_genet_layout_planes_architecture.md)) | genet-layout = Stylo (style) + Taffy (box) + parley (text) | each plane swappable in principle; Taffy is the box-layout provider |
| **Cross-engine observables** | `engine-observables-api` (proposed, hekate doc) | Nematic / Genet / Scrying lanes | each lane implements the subset it has |
...
1. **Reflector-composition (adopted).** DOM arena (Genet's) and JS heap (Nova's) stay
...
2. **Shared substrate (speculative).** DOM nodes and reflectors both backed by one
   Genet "house arena" lib (the layout_dom_api candidate pattern), Nova's core heap
...
gating.** The DOM arena is always Genet's and always present; the *reflector half* of
a node is what's gated. Compile Nova out (`genet-static-dom`) → reflector field and
the `nova_vm` dep vanish, pure DOM arena remains. Compile it in (`genet-scripted-dom`)
...
> in the [profile-ladder plan](./2026-05-12_genet_profile_ladder_plan.md) and the
...
cargo tree -p genet-static-html  | rg "script-engine|mozjs|boa_engine|rquickjs|nova"   # expect: no matches
cargo tree -p genet-interactive-html | rg "script-engine|mozjs|boa_engine|rquickjs|nova" # expect: no matches

# wasm scripted graph excludes spidermonkey/mozjs
cargo tree -p genet-scripted --target wasm32-unknown-unknown | rg "mozjs|spidermonkey"   # expect: no matches

# each backend builds in isolation
cargo check -p script-engine-quickjs
cargo check -p script-engine-boa
cargo check -p genet-prerender --features quickjs
cargo check -p genet-prerender --features boa
```
...
cargo test -p genet-prerender --features quickjs -- ssr_fixtures
cargo test -p genet-prerender --features boa     -- ssr_fixtures
...
- **Re-marrying the DOM to one engine.** The whole point of the cut is the
  ...
  into `layout-dom-api` or `genet-scripted-dom`'s *public* surface re-creates the
  772-file problem. Engine types live behind the `script-engine-*` impls (the
  `new_function` lowering) only — never in `layout-dom-api`, `genet-scripted-dom`,
...
2. **Microtask/event-loop ownership for live scripting.** Prerender pumps to
   ...
   event-loop *driver* sits in `script-runtime-api` itself or in `genet-scripted`
...
3. ~~**Backend default per target.**~~ **Resolved 2026-05-21 (Part 6):** Nova is
   ...
   reversible.
4. **Does prerender warrant its own profile-ladder tier** (`genet-prerender`
   between interactive and scripted), or is it a *mode* of `genet-scripted` that
...
- If the VM primitive surface (esp. `new_function` + captured-state GC integration)
  ...
  per-backend `genet-scripted-{quickjs,boa}` crates with no shared trait (rakers'
...
- **Proposed 2026-05-20:** resolves layout_dom_api open question #1 — DOM mutation
  is a `LayoutDomMut: LayoutDom` extension trait on a new `genet-scripted-dom`
  provider. The provider emits render-state-free `DomMutation` records (no
  `mark_dirty`); genet-layout's scheduler translates them into invalidation. The
...
- **Proposed 2026-05-20:** WPT coverage is tracked on **two axes** — engine
  ...
  event-loop + platform conformance (WPT, Genet's work). No `min()` formula:
...
- **Prerender sandbox is mandatory for remote content.** Hostile remote JS runs in
  ...
  affordance only.
- **Prerender is a Genet render-tier selection, not a Hekate E3 extraction mode.**
  Hekate's app-shell/empty-root signals are tier-selection *evidence* passed as a
  hint; Genet owns the tier decision. Keeps the E0–E4 extraction vocabulary clean.
...
- **Track A built and green (2026-05-22)** at `probes/script-engine-probe/` (standalone
  workspace, isolated from genet's audit gates). `ScriptEngine` + `ScriptEngineLive`
...
- **Track B2 patch scope pinned from source (2026-05-22).** Read
  ...
  **(4)** the `usdt` wasm target-gate (Appendix B). ~1 field + 3 small method bodies +
  2 pub fns. Apply on the `genet-embedder` branch; consume via `[patch.crates-io]`.
- **Track B2 patch landed and green (2026-05-22).** The patch (above) is implemented on
  branch `genet-embedder` of `repos/nova` (commit `fbca54b`): `embedder_data: u64` slot,
...
- **Part 3 — probes lifted to real genet crates (2026-05-23).** Three crates under
  ...
  **`script-engine-nova`** (native-only via `cfg(not(wasm32))`, `nova_vm` redirected to the
  ...
  api, nova, **and now boa** — are genet **workspace members and green**
  (`script_engine_nova` reflector round-trip survives GC in-workspace).
- **RESOLVED — Boa joined genet's graph via a fork (2026-05-25).** *Prior blocker
  (2026-05-23):* `boa_engine 0.21.1` hard-deps `icu_normalizer ~2.0.0` (non-optional);
  genet's **parley 0.9** needs `^2.1.1` — disjoint, so adding boa broke workspace
  ...
  `crates/boa` (`genet` branch, shallow @ `v0.21.1`/`bc36c3f`), widened the icu family
  pin `~2.0` → `^2.1` (one workspace `Cargo.toml` edit), and redirected `boa_engine` +
  `boa_gc` via genet's root `[patch.crates-io]` (same pattern as `nova_vm`).
...
- **genet-scripted-dom — foundation built + green (2026-05-23).** Resolves
  ...
  `components/genet-scripted-dom` (workspace member): a mutable `NodeId` arena
  ...
  **Next pass (flagged, not done):** (1) `set_inner_html` (needs html5ever fragment
  ...
  through the `script-runtime-api` host layer; (3) the **`DomMutation` → genet-layout**
  invalidation loop (genet-layout's scheduler consuming the drained stream). The probes
...
- **Reflector bridge green — JS→DOM works end-to-end (2026-05-23).** New
  `components/genet-scripted` (workspace member, native-only): a JS script handed a
  ...
  **This is the binding layer (the "hard 90%") demonstrated end-to-end**, leak-free: the
  ...
  host layer. Next: **#2** `DomMutation` → genet-layout invalidation (assess the
  scheduler seam first), then **#3** `set_inner_html`.
- **#2 assessed (2026-05-23): no incremental seam — it's a subsystem, not a wire.**
  genet-layout is generic over `D: LayoutDom` (so it *can* lay out a `ScriptedDom`), but
  ...
  integration needing genet-layout's font/viewport/pipeline setup; not a quick wire);
...
- **#3 done — `set_inner_html` green (2026-05-23).** Added `set_inner_html` to
  ...
  genet-scripted-dom by reusing genet-static-dom's parser: parse the fragment into a
...
- **#2 direction (2026-05-23): chose (b) incremental, with (a) coarse as the
  ...
  scripting; (b) leans on Stylo's *existing* invalidation that genet merely stubbed). The
...
- **#2(a) coarse relayout oracle — DONE, green (2026-05-23).** Added `genet_layout::render`
  (a viewport-typed wrapper over cascade→layout, generic over any `LayoutDom`) and
  `genet_scripted::relayout_if_dirty` (drains the DOM's `DomMutation`s; if non-empty,
  ...
  over a `ScriptedDom`** — genet-layout is generic over `LayoutDom`, no font setup needed.
  One real constraint surfaced + fixed: genet-layout's Stylo style-sharing cache asserts
...
- **#2(b) incremental — assessed as a major subsystem; recommend a focused arc, not a tail
  task (2026-05-23).** It is *not* a wire or a single function: genet-layout currently
  *rebuilds everything every pass* (`construct` builds a fresh Taffy tree; `run_cascade`
  ...
  traversal genet no-op'd) for partial restyle; (3) **incremental `construct`/Taffy**
  (cache the tree, `mark_dirty` only affected nodes — genet-layout has no partial-layout
  entry today); (4) partial paint. Each touches genet-layout's core. The **diff-test
...
- **#2(b) core BUILT + diff-tested (2026-05-24).** The incremental pipeline exists and
  ...
  - **Plan** — `genet_layout::classify(DomMutation) → Invalidation{RestyleSubtree |
  ...
  - **Execute** — `genet_layout::SubtreeView` re-roots a `LayoutDom` so the existing
    pipeline lays out just one subtree; `render_subtree` wraps it.
  - **Splice** — `genet_scripted::relayout_incremental(dom, prior, …)` drains → classifies
...
- **Fine-grained restyle — investigated; it's the deliberately-stubbed Stylo invalidation
  ...
  **Precise plan (execute in a focused Stylo session):**
  1. **Snapshots** — before a mutation, capture the element's old attrs/classes/state into
     the `SnapshotMap` (hook genet-scripted-dom's `set_attribute`/structural mutators);
...
```rust
// Used by genet-scripted-dom only; prerender never sees it.
...
`genet-embedder` carries only the EmbedderObject reflector patch.
...
- **Mechanism:** the diff lives on a thin branch of `github.com/mark-ik/nova` (e.g.
  `genet-embedder`), consumed via `[patch.crates-io] nova_vm = { git = "...mark-ik/nova",
