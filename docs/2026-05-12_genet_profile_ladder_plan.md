...
crates inside Genet). The *implementation* direction has gone through three
reframings since 2026-05-12:

1. **2026-05-15 audit** moved `components/layout/` and `components/script/`
   to dead-on-disk; `genet-layout` becomes a new crate.
2. **2026-05-16 lift plan** ([2026-05-16_genet_layout_lift_plan.md](./2026-05-16_genet_layout_lift_plan.md))
   proposed path C (lift portable layout into `genet-layout`).
3. **2026-05-17 planes architecture** ([2026-05-17_genet_layout_planes_architecture.md](./2026-05-17_genet_layout_planes_architecture.md))
   is the **current authoritative target shape**: Stylo + Taffy + parley +
   selected lifted display-list machinery, with `genet-layout`-owned planes
...
**Reconciliation note (2026-05-21):** the JS-engine slot in the `genet-scripted`
tier is now **Nova** (primary, pure-Rust) with Boa as a conformance oracle — see
[script-engine plan Part 6](./2026-05-20_genet_script_engine_plan.md). This
...
[archive/2026-05-05_genet_netrender_cut_plan.md](./archive/2026-05-05_genet_netrender_cut_plan.md)
...
goal is to make Genet itself compile from a Blitz-like minimum shape up
...
- **Genet static HTML** handles static/simple HTML and CSS through the
  smallest Genet profile.
- **Genet fullweb** handles JS-heavy pages, browser APIs, workers,
...
capability need, then let Genet scale internally rather than creating a
...
- the all-up browser facade is one composition, not Genet's minimum
  identity;
...
- Added `components/genet-static-html` as the first profile witness
  package.
- Added `components/genet-static-dom` as the first script-free HTML5
...
- Removed direct `script_traits` dependencies from `servo-layout` and
  `servo-layout-api`.
- Wired `genet-static-html` to parse HTML into `StaticDocument`.

Receipts:

```powershell
cargo check -p genet-static-dom
cargo test -p genet-static-dom
...
- Make `genet-static-dom` implement the provider as the second provider.
...
Status: implemented for `genet-static-html`.
...
- Add placeholder package targets or documented `cargo check` aliases for
  `genet-static-html`, `genet-interactive-html`, `genet-scripted`, and
  `genet-fullweb`.
...
```powershell
cargo check -p genet-static-html
cargo tree -p genet-static-html | rg "script|script_bindings|mozjs|servo-media|storage"
cargo check -p genet-fullweb
```
...
Status: started. `genet-static-dom` now parses HTML through html5ever
into a script-free `StaticDocument`; it does not yet implement the
layout input traits.

This is where Genet becomes Blitz-like without using Blitz as the
runtime architecture.

Work:

- Add a `genet-static-dom` package or module.
...
HTML string/file
    ...
    -> Servo style/layout
    -> Genet display list / paint_types
    -> servo-paint
    -> NetRender readback
```
...
- `cargo check -p genet-static-html` succeeds without `script` or
  `mozjs`.
...
- Choose one of two shapes:
  - **Preferred implementation shape:** create profile facade packages
    (`genet-static-html`, `genet-fullweb`, later
    `genet-interactive-html` / `genet-scripted`) and let
...
- Let `genet-static-html` use a direct document pipeline at first:
...
```powershell
cargo tree -p servo-layout | rg "script|script_traits|script_bindings|mozjs"
cargo tree -p genet-static-html | rg "script|script_traits|script_bindings|mozjs"
```
...
- The only validating gate that exercises the trait is
  `NoOpLayoutHostServices` (trivially `Sync`), which `genet-static-html`
  uses.
