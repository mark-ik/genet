# Ring 3 unwalling: serval-stylo + serval-taffy, and the taffy ride-out

**Date:** 2026-07-12
**Status:** **T0, T1, T2, and T3's manifest sweep all landed 2026-07-12.**
serval's workspace now builds, tests, and passes all nine WPT baselines
against: taffy re-vendored to stable 0.12.1 (T0), the renamed stylo fork
family on branch `mark-ik/serval-publish-names` (T1, commit `efaa436663`),
...
**Not done:** the taffy package itself is not yet renamed to `serval-taffy`
...
`2026-07-11_serval_publish_rings_plan.md` (this is the "ring 3 publishes
...
**serval-stylo** (family) and **serval-taffy** — while working taffy back to
...
`serval-stylo`, `serval-taffy`, `serval-stylo-atoms`, `serval-stylo-dom`,
`serval-stylo-taffy`, `serval-stylo-traits`.
...
| --- | --- | --- |
| `style` (pub. `stylo`) | ~1.8k lines: Tier D/E media features, multi-capability pointer/hover, forced-color-adjust (+revert), the f32 animation-boundary fix | **rename: `serval-stylo`** |
| `stylo_atoms` | +334 (media-feature atoms) | **rename: `serval-stylo-atoms`** |
| `stylo_dom` | +199 | **rename: `serval-stylo-dom`** |
| `stylo_static_prefs` | +208 | **rename: `serval-stylo-static-prefs`** |
...
Keep `links = "servo_style_crate"` on serval-stylo: it makes cargo refuse
any graph that accidentally pulls registry `stylo` alongside it — the
silent-upstream-resolution trap caught by the serval-static-html
...
- **0002 exclusion-band accessor** — still absent; additive; consumed by
  serval-layout's parley IFC seam.
...
upstream-differing files taken as-is; only the 5 files the three serval
...
carried over unchanged in substance. `content_size` (a taffy feature serval
...
all four consumers' feature lists (`paint`, `serval-layout`, `serval-render`,
`serval-wpt`) — the one real gap the re-vendor surfaced.

**Verified:** serval-layout 320 tests, the html-to-pixels paint corpus (30),
`serval-xilem` (101), `serval-scripted` (45), and all nine WPT baselines (7
...
Branch `mark-ik/serval-publish-names` (commit `efaa436663`, pushed):
`serval-stylo` / `serval-stylo-atoms` / `serval-stylo-dom` /
`serval-stylo-static-prefs`. Workspace checks clean, both standalone and
(later, per T3) as consumed by serval.
...
`TrackBreadth::Flex` back to serval fork's `TrackBreadth::Fr` shape (a
registry-`stylo`-lineage rename stylo_taffy's authors made that serval's
...
must name the *real* renamed packages (`serval-stylo`/`serval-stylo-atoms`)
...
simpler than the original plan assumed. Renaming taffy to `serval-taffy` is
...
`serval-stylo`'s real package name now differs from what every consumer's
`Cargo.toml` key says) flips that: the extern name becomes the **TOML key**
instead. `stylo`'s package is renamed to `serval-stylo`, but its `[lib] name`
...
Adding the necessary `package = "serval-stylo"` to the `stylo = {...}`
...
serval workspace, post-sweep: `cargo check --workspace` (exit 0); test
suites — serval-layout 320, paint html→pixels 30, `serval-xilem` 101,
serval-scripted 45; WPT — all nine baselines (`dom`, `css_animations`,
...
- **meerkat (mere) build is currently red against this work — a cross-repo
  ...
  pins serval by `git … branch = "main"`; the rings 0–2 lane's opportunistic
  ...
  *intermediate* snapshot of the stylo_taffy vendor — mid-edit, before the
  ...
  next full sweep/push of serval's `main`; no action taken here to force a
  push, consistent with how commits to serval have been handled all session.
- **`examples/serval_web_smoke` is broken independent of this work.** Its own
...
- The script cone: boa fork, nova fork (`nova_vm` name is fork identity —
  vano posture), serval-scripted, and therefore serval-documents'
