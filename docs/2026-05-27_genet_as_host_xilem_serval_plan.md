# genet as host: the `xilem_serval` reactive backend

Status: **strong through Stages 0-7; the previously named host-backend blockers
are now landed.** Scopes using
genet as the application host (chrome and content rendered by one engine),
...
`xilem_core` diff → genet DOM → layout → paint → netrender → present, with
input routed back through genet's hit-test + faithful xilem message
dispatch — is validated on screen (`pelt-live-counter`), genet the sole
...
native dispatch substrate, which wires genet's *existing* hit-test query
into event routing rather than building a new one. Per-stage status and
commits are in [Staging](#staging).

## The three genet-GUI architectures

"Use genet for the GUI" is three different architectures, and most
confusion comes from blurring them.

1. **genet-as-texture-in-a-host.** genet renders a document to a wgpu
   ...
   The host owns the chrome; genet owns the content. This is the right
   division of labor while Xilem works, and nothing here proposes
   changing it.
2. **host framework + genet-rendered chrome (excluded).** Render the
   chrome *through* genet while keeping the host framework's widgets.
...
3. **genet-as-the-host.** genet owns the window, layout, paint, input,
   script, and accessibility. Chrome and content are the same kind of
   object: documents. This is the Blitz/Dioxus model, and genet already
...
hit-testing. So taking *only* that piece and pointing it at genet's
[`LayoutDomMut`](../components/shared/layout-dom/lib.rs) keeps genet the
...
at genet's DOM, with `xilem_web` as the line-for-line template.

The mental model: **`xilem_serval` is `xilem_web`, but native, with
genet as the engine you own.** Xilem's Rust reactive authoring, genet's
web-fidelity rendering, one stack end to end (Xilem authoring → genet
...
which is a slightly better impedance match to genet's mutation-recording
...
## Mapping the contract onto genet

| `xilem_web` (browser) | `xilem_serval` (genet) | status |
| --- | --- | --- |
...
| browser hit-test + `addEventListener` | `GenetLaneView::hit_test` exists; **native dispatch + listener registry** not wired | gap (wiring, shared) |
...
[`ScriptedDom`](../components/genet-scripted-dom/lib.rs) need:
...
`xilem_serval` has no browser, but genet is further along than "no
hit-test." [`GenetLaneView`](../components/genet-layout/genet_lane.rs)
...
1. **Identity round-trip.** `hit_test` returns a `SourceNodeId` (genet's
   `opaque_id`), and the reverse `SourceNodeId → NodeId` lookup in
   `GenetLaneView` is currently an O(n) DOM walk. Native dispatch wants
...
3. **The window → lane wiring.** A pelt host pointer event feeds
   `GenetLaneView::hit_test`; the hit maps back to a node; the dispatch
...
framing: genet already has the hit-test query; Stage 2 *promotes and
...
**Same engine does not mean same DOM tree.** genet-as-host means one
...
**A genet-native runner owns scheduling.** `xilem_core` provides view
diffing and message thunks, but it schedules nothing. Something
genet-native has to own state, the root node, the message queue, timers
and microtask checkpoints, render invalidation, and the rebuild cadence. A
small `GenetAppRunner` is therefore a Stage 1 *artifact*, not an
...
- **Stage 1b — done, decomposed into core + window.**
  - **1b-core (`2e4c2e8`):** `GenetAppRunner` (the real artifact —
...
- **Stage 2 — done, in two slices.**
  - **2a (`ff22abc`):** the `point → NodeId` half, wiring genet's existing
    `GenetLaneView::hit_test` (no new spatial index). The reverse
  ...
  - **2b (`9c01c27`):** faithful event dispatch (the chosen path over a
    ...
    registers its `view_path` in `GenetCtx`; `dispatch_click` bubble-walks
...
- **Stage 3 (breadth) — partly done.** Grows from "counter" toward
  ...
  - **Component composition + Action-bubbling — done (`84fceae`).**
    ...
    drive `GenetCtx` with **zero** backend impls (the identity
  ...
  - **Keyboard + focus — done (`09f2bf1`).** `on_key` (the faithful-routing
    twin of `on_click`), a genet-native `KeyEvent`/`Key`/`NamedKey`, a key
    registry on `GenetCtx`, runner `focus` with click-to-focus, and
  ...
  - **Live a11y adapter — done.** `accesskit_winit::Adapter` is wired into
    ...
    screen reader.
  - **Real caret painting — done.** `genet_layout::caret_rect` /
  ...
  - **`Element` / `Text` split — done.** An `ElementView` marker gates
    ...
    node stays one runtime type (`GenetElement`), so children mix text +
    elements freely.
...
- **Stage 4 (overlays + inline style) — done.** The host-overlay slice, and
  ...
  - **Inline `style` support (`dfe8702`).** genet's stylo adapter returned
  ...
  - **Overlay primitives (`4bf92fb`).** `overlay_at(x, y, content)` — a
    ...
    laid-out rect, since only the host has layout). The caller owns two genet
...
- **Stage 5 (erased views + `select`) — done.** The first composable control on
  ...
  a named app.
  - **Erased views / `AnyView` (`b6872ff`).** `impl AnyElement for GenetElement`
...
- **Stage 6 (scrolling) — done.** A Lane H axis, wired from existing netrender
  ...
  - **Clip-aware hit-testing — done.** The first naive scroll hit-test leaked
    through clipped content. `GenetLaneView::hit_test` now respects overflow
...
- **Stage 7 (z-index Tier 1 + remaining form controls) — done.** Scoped in
  ...
  - **textarea (`99998dae76a`).** `textarea` over the existing `TextInput`:
    Enter → `\n`, Up/Down line navigation, line-scoped Home/End. genet feeds
...
Mere's [genet-as-host decision brief](../../mere/design_docs/mere_docs/technical_architecture/2026-05-29_genet_as_host_evaluation.md)
gates flipping Mere's chrome onto genet on genet reaching Masonry's
...
IME pays once for the whole engine: genet needs it for content text entry
...
genet already caches the `parley::Layout` per text leaf
([text_measure.rs](../components/genet-layout/text_measure.rs)); the
...
so it is the natural next genet-layout addition when this gate work starts —
and it is the reason caret painting is grouped here rather than treated as a
cosmetic one-off.

## What genet makes simpler

`xilem_web` defers prop application to `PodMut::drop` to batch DOM writes.
genet already batches at the `drain_mutations` → relayout boundary, so
...
Accessibility is also more natural here: genet emits `AccessKit` from a
...
`pelt` example host. genet already depends on the `crates/xilem` fork
...
- The **Blitz/genet convergence** thesis (genet as a Blitz-shaped
...
- [2026-05-25_js_execution_strategy.md](./2026-05-25_js_execution_strategy.md)
  ...
  and both drive the same genet DOM through separate roots.

## Non-goals

- Replacing the Xilem-chrome + genet-content split (architecture 1). That
...
- Rendering chrome through genet on top of a host framework
