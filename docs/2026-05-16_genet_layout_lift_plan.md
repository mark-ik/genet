# genet-layout lift plan (path C)

Implementation plan for path C of the profile ladder: lift the portable parts of dead-on-disk `components/layout/` into a new `genet-layout` workspace crate, with `genet-static-dom` as the first DOM provider plugged in behind a profile-neutral `LayoutDom` trait.

**Anchors:**

- Strategy: [2026-05-12_genet_profile_ladder_plan.md](./2026-05-12_genet_profile_ladder_plan.md) (profile ladder framing — still canonical as strategy).
...
- [2026-05-17_genet_layout_planes_architecture.md](./2026-05-17_genet_layout_planes_architecture.md) — `genet-layout`'s piece (Style + Layout + Fragment + Paint planes for HTML, publishing the FragmentQuery + InteractionQuery surfaces).
- [2026-05-17_hekate_lanes_observables.md](./2026-05-17_hekate_lanes_observables.md) — the **cross-engine** architecture. Hekate is a router + document-intelligence layer, not a renderer; lanes are Nematic / Middlenet / Genet fullweb / system-webview-fallback; extract is Hekate's own work with E0–E4 tiers; observable-plane vocabulary is shared across lanes. Genet is **one lane** in this system.
...
- **Mutable rendering state lives in planes**, not embedded on DOM nodes. Each plane is keyed by `D::NodeId` and owned by `genet-layout`. Multi-DOM-provider goal preserved (static / scripted / reader-mode all use the same planes); DOM crates stay clean.

**Lift size revised:** ~5.5k lines lifted (display_list + style helpers + WM geometry + list markers + replaced sizing + quotes), plus ~3-4k lines of new plane construction / Stylo adapter / Taffy glue / parley wiring. Net `genet-layout` size: ~8-10k lines, down from the path-C plan's ~30k.
...
- `components/shared/layout-dom/` exists and `genet-static-dom::StaticDocument`
  implements `layout_dom_api::LayoutDom`.
- `components/genet-layout/` exists, builds, and has a structural
...
## How path C lines up with broader genet goals

Cross-checking the lift against the strategic anchors:

| Goal | Path C alignment |
| --- | --- |
| Profile ladder (static → interactive → scripted → fullweb) | Direct. `genet-layout` becomes the shared middle layer above static-dom and (future) scripted-dom providers. |
| Three-head Hekate (extract / middlenet / fullweb) | Middle and full heads share `genet-layout`. Extract head bypasses it for readability-style text flow. Mark divergence points during the lift; don't refactor later. |
| Shared NetRender output across profiles | `genet-layout` emits `GenetDisplayList`. `servo-paint` consumes it. Output target unchanged. |
| Blitz convergence | The end shape (Stylo + Taffy + genet-internal box/fragment trees + display list emission) is the same general shape as Blitz's `packages/dom + packages/stylo + packages/taffy_layout + packages/paint`. A side-by-side audit becomes meaningful once the lift is done. |
...
| Wasm/browser target (eventually) | Lift must stay wasm-friendly. Don't pull native-only sync primitives or threading assumptions into `genet-layout`. Audit `Send + Sync` decisions explicitly. |
...
| Mere ecosystem fit (inker routes to genet) | Strengthened — inker can carry a "profile preference" payload to genet per route once the profiles are real packages. |
...
Done condition: `cargo check -p genet-layout` succeeds, and the package-name
canary for `genet-layout` returns no `servo-script`, `servo-script-traits`,
...
`genet-static-dom`'s `StaticDocument` implements `LayoutDom` immediately as the validation.

Done condition: `genet-static-dom` exposes a `LayoutDom` impl; `cargo check -p genet-static-dom` succeeds; a unit test in `genet-layout` exercises the trait against `StaticDocument` (e.g., walks the tree and reads element names).

**P2.3 — Port layout core.** Files in `components/layout/` move into `components/genet-layout/`, with each file's `script::layout_dom::*` references replaced by `LayoutDom`-shaped trait calls. The original low-coupling order below is now retained as a cautionary record, not as the live execution rule:

1. `geom.rs`, `layout_box_base.rs`, `cell.rs` — close to pure math/data.
2. `display_list/` — emits `GenetDisplayList`; minimal DOM touch.
...
Done condition for P2.3: `genet-layout`'s public API covers what `genet-static-html` needs to layout a parsed `StaticDocument`.
...
**Strategy 1 — Cluster by layer.** Port multiple files together as one logical layer per commit (e.g., "geometry + ContainingBlock + Size as a single 'core geometry/sizing types' batch"). Each commit still has `cargo check -p genet-layout` green; commits are larger but the dependency-graph reality is honored. Maybe 6–10 commits total for P2.3. Bisect-friendly *between layers*, not between files.

**Strategy 2 — Jump-ship single-commit port.** Move all of `components/layout/` to `components/genet-layout/` in one operation; fix what breaks; one commit when done. The "rip the parallel codepath, fix what breaks" model the netrender cut used. Mark's documented preference for prototypes ([feedback_jump_ship_over_migration_for_prototypes]). One big commit; less bisect-able mid-port, but the audit canary is binary — either the final state is script-free or it isn't. Faster.
...
- ✅ **Batch 1a (2026-05-16):** `cell.rs` ported. `genet-layout` builds; audit canary empty.
...
- 🟢 **Review validation (2026-05-17):** `cargo check -p genet-layout`, `cargo test -p genet-layout`, `cargo check -p genet-static-dom`, and `cargo check -p genet-static-html` pass. The corrected package-name canary returns no script / mozjs packages for both `genet-layout` and `genet-static-html`.

#### P2.3 next executable slice — adapter compile gate (superseded 2026-05-17)

**[2026-05-17: this slice's target shrinks under the planes architecture.](./2026-05-17_genet_layout_planes_architecture.md) The Stylo adapter still gets written (now living in `genet-layout/src/stylo_adapter/`), but the four `layout_api` trait impls (LayoutNode / LayoutElement / DangerousStyleNode / DangerousStyleElement) and `LayoutDomBundle` are no longer needed — Blitz's read showed that scaffolding is Servo-internal and serves no consumer outside Servo's layout. The probe slice now also wires Taffy + parley + a StylePlane skeleton. See the planes doc for the full slice scope. The section below is preserved as the original gate framing.]**
...
1. **Remove accidental direct dependency debt first.** If `genet-layout` does
...
2. **Split adapter work into two files.**
   ...
   This keeps the Genet-facing adapter boundary readable instead of burying it
...
```powershell
cargo check -p genet-layout
cargo test -p genet-layout
cargo tree -p genet-layout --edges normal | rg '(^|[[:space:]])(servo-script|servo-script-traits|servo-script-bindings|mozjs|mozjs_sys)([[:space:]]|$)'
...
First attempt: bulk-moved all of `components/layout/` (minus `table/`, `taffy/`, `tests/tables.rs`, the `layout_provider.rs` stub) into `components/genet-layout/`, ported the Cargo.toml (drop `script` + `taffy`, add `layout_dom_api`), dropped the `mod table` / `mod taffy` / `mod layout_provider` declarations from `lib.rs`, replaced the layout_provider stub with an empty placeholder.

**`cargo check -p genet-layout` after the bulk move: 25 errors.** Sorted by class:
...
1. Becomes generic over `D: LayoutDom` (and its associated `LayoutNode` impl), propagating `D` through `Layout<D>`, `BoxTree<D>`, `Fragment<D>`, etc. Stylo's `TElement` shim is a per-D adapter struct in `genet-layout`.
2. Or: stays concrete by picking a single `D` at lift time (e.g., `genet-static-dom::StaticDocument`), which defeats the abstraction (layout becomes coupled to one DOM impl; reader-mode head can't reuse it).
...
**P2.4 — Delete `components/layout/` and `components/script/` from disk.** Same commit series. No "keep around just in case" — two layout implementations is worse than one. The genet audit snapshot's dead-on-disk list (`components/net`, `components/devtools`, `components/storage`, etc.) can come along in a sweep, but `layout/` and `script/` are the load-bearing ones for this phase.

### P3 — Static HTML first-pixel smoke

`genet-static-dom` parses HTML. `genet-static-html` is currently a witness that constructs a `StaticHtmlProfile` and parses; it does not invoke layout. After P2.3, wire the pipeline end-to-end:

```text
HTML string
  → genet_static_dom::StaticDocument::parse(html)
  → genet_layout::layout(&doc, &host_services, viewport)   // emits GenetDisplayList
  → servo_paint::Paint::render(&display_list)
  → NetRender readback to RGBA8 buffer
```

The smoke test renders a single CSS-styled paragraph and asserts the readback has non-background pixels in the expected region. Lives in `components/genet-static-html/tests/first_pixel.rs`.
...
- A `genet-scripted-dom` placeholder crate that documents the intended shape.
- A `genet-scripted` profile facade placeholder (analog to `genet-static-html`).
- A note in the strategy doc about which Servo script-DOM commit/tag the future port will start from.

No code paths from `genet-scripted` get wired up. This phase exists to make the eventual reintroduction non-mysterious and to surface the `LayoutHostServices` `Send + Sync` decision before it bites.

### P5 — Profile facade packages

Already shipped for `genet-static-html`. Add `genet-interactive-html` and `genet-fullweb` as placeholder crates with documented intent and `cargo check`-green empty libs. Wire `support/profile-gates/` to assert that each profile's `cargo tree` doesn't carry capabilities above its level (e.g., `genet-interactive-html` doesn't carry `mozjs`).

### P6 — Low-profile pipeline split

After P3 lands, `genet-static-html` has a direct pipeline that doesn't need `components/constellation/` lifecycle. Confirm this. Don't extract a shared `pipeline-core` until/unless fullweb is alive and we can prove the overlap.

### P7 — Wasm host (later)

Unchanged from strategy doc. Kept here so the lift's wasm-friendliness assumptions are pinned: no native-only sync primitives in `genet-layout`, no `pollster::block_on` startup in any path that wasm builds reach.
...
- **Blitz side-by-side audit.** Now feasible per the audit snapshot — genet's shape is finally narrow enough to lay next to `linebender/blitz`'s `packages/*` and read the overlap. Specifically, their `packages/dom` (DOM abstraction), `packages/stylo` (Stylo integration), `packages/taffy_layout` (Taffy wrapping), and `packages/paint` (display-list lowering) map to where `genet-layout` is heading. Likely surfaces pieces of Blitz worth pulling rather than porting; informs the per-file judgment of what counts as "portable."
...
- **Stylo pin discipline.** Stylo isn't on crates.io. It comes through `workspace.dependencies` as `stylo = { ... }` — likely via path or git dep through servo-stylo. During the lift, audit how `stylo` resolves and pin the source explicitly so `cargo update` can't silently move genet onto a Servo branch we didn't expect. (Workspace pins drive probes; the rule applies here.)
...
- **`layout_api` cleanup pass.** The interface crate still carries `Painter` / `DrawAPaintImageResult` / `PaintWorkletError` that the static profile will never need. Decide whether to feature-gate these or split them into a fullweb-only API crate. Affects whether `genet-layout` can be built with no paint-worklet code paths at all (W3C-knockout pattern).
- **`servo-` package prefix rename.** Not blocking, but when introducing `genet-layout`, set the precedent: `name = "genet-layout"`, not `name = "servo-layout-v2"`. The 56 remaining `package = "servo-..."` workspace entries can follow in a subsequent rename pass.
- **Reach-readability decision.** The static profile renders raw HTML; the smolweb-extract head will use readability heuristics. Sometime during P3, decide whether readability extraction lives in `genet-static-dom` (preprocessing) or in a separate `genet-reader-dom` crate. Affects how `nematic` and `genet` divide work for reader-mode requests.

---

## Pitfalls to keep in view

- **Re-introducing the SpiderMonkey env requirement.** `genet-layout`'s deps must be vetted at each step. Adding a transitive crate that depends on `servo-script` / `servo-script-traits` / `servo-script-bindings` / `mozjs` would silently re-introduce NASM / MOZILLABUILD / clang-cl. The canary must match package names, not the substring `script`, because clean graphs contain crates like `unicode-script`. Single line in `workspace.exclude` separates us from the world of NASM.
...
- **Paint Worklet / CSS Houdini Painter.** `Painter` / `PaintWorkletError` already live in `layout_api`, but only fullweb implements the trait. The static profile must never invoke a worklet. Make sure `genet-layout` either feature-gates the worklet code path or stubs it to `unreachable!()` — the canary is `cargo tree | rg paint_worklet`.
...
- **Ship of Theseus during the port.** "Portable" is a judgment call per file. Make each port a separate commit with `cargo check -p genet-layout` green and the profile-gate canary empty at every step. Bisect-friendly history is the only way to catch a silent script-coupling reintroduction during a multi-week port.
...
- **Don't conflate "profile-neutral DOM" with "Servo's DOM minus script."** Servo's DOM is JS/reflector/GC-shaped at its core. The `LayoutDom` trait should be designed *outward* from what `genet-layout` actually consumes, not *inward* from what Servo's DOM happens to expose. If the trait looks like Servo's DOM, the scripted-DOM impl will be trivial but the static-DOM impl will have absorbed Servo-DOM-shaped complexity for nothing.
...
cargo tree -p genet-layout --edges normal | rg '(^|[[:space:]])(servo-script|servo-script-traits|servo-script-bindings|mozjs|mozjs_sys)([[:space:]]|$)'
cargo tree -p genet-static-html --edges normal | rg '(^|[[:space:]])(servo-script|servo-script-traits|servo-script-bindings|mozjs|mozjs_sys)([[:space:]]|$)'
...
```powershell
cargo test -p genet-static-html --test first_pixel
...
cargo check -p genet-layout
cargo check -p genet-static-html
cargo tree -p genet-layout --edges normal | rg '(^|[[:space:]])(servo-script|servo-script-traits|servo-script-bindings|mozjs|mozjs_sys)([[:space:]]|$)'        # empty
cargo tree -p genet-static-html --edges normal | rg '(^|[[:space:]])(servo-script|servo-script-traits|servo-script-bindings|mozjs|mozjs_sys)([[:space:]]|$)'   # empty
cargo test -p genet-layout                                                            # green
cargo test -p genet-static-html --test first_pixel                                    # green
...
7. **`net_traits` boundary** — current graph reaches `servo-net-traits` through `layout_api`, and `genet-layout` currently has a direct entry too. Decide whether the direct edge is removable now; decide later whether `layout_api` should split image/media-facing pieces so the static profile can become net-traits-free.
8. **Capability traits — crate location.** **New (2026-05-17):** `ReplacedElementProvider` / `FormControlProvider` / `EmbeddedContentProvider` live in `layout_dom_api`? Sibling `layout_capabilities_api`? Inside `genet-layout`? See planes doc; lean separate `layout_capabilities_api`.
9. **Plane visibility.** **New (2026-05-17):** are planes (`StylePlane`, `LayoutPlane`, `FragmentPlane`) public read-only API of `genet-layout` (so mere/apparatus/a11y can read FragmentPlane directly), or `pub(crate)`? Lean `pub` reads — "planes as reusable observables" framing depends on it.
10. **Display-list lift mechanism.** **New (2026-05-17):** Servo's `display_list/` consumes fragment-tree-shaped types (`Fragment`, `BoxFragment`). Define equivalents in `genet-layout::fragment` for high lift fidelity, or refactor the lifted display-list to read plane data directly? Lean equivalents — preserves lift value at small cost.
