# serval holistic audit (2026-06-02)
...
[2026-05-29 audit](./2026-05-29_serval_holistic_audit.md); companion to the
[two-lanes operationalization](./2026-05-29_serval_two_lanes.md).

## The one-line thesis (unchanged)

serval converges two arcs on one substrate. Arc A is web conformance (run the
real web: testharness, DOM, eventually fullweb). Arc B is serval-as-host
(serval's own DOM as native Xilem-driven app UI, chrome as CSS). Both ride one
DOM (`serval-scripted-dom` + `LayoutDomMut`), one event model
...
- **Layout: strong, including flex / grid / positioned.** Block, inline,
  ...
  serval-owned unit/reftest; correctness rests on taffy upstream plus the
...
- **Serval-as-host (Arc B): now real, not a counter demo.** Three mere crates
  ...
  `RepaintOnly` path), and **serval-winit-host** (the shared winit+netrender
...
- **Two development lanes, one spine** is the canonical operationalization
  ([2026-05-29_serval_two_lanes.md](./2026-05-29_serval_two_lanes.md)), distinct
  from Hekate engine-routing lanes (Nematic / Serval / Scrying).
...
- **The two-masters DOM boundary is enforced by the embedder, not the
  ...
  But `serval-scripted-dom` has no type-level or runtime fence; handing both arcs
...
- **There is no CI in the serval repo.** The only tier gate is a manual
...
- **Dead-on-disk crates remain** (OHOS media, `shared/bluetooth`,
  ...
  cleanup (it is live).
- **`customElements` has no upgrade/lifecycle**; flex/grid have no serval-owned
...
- **Take (cheap, dual-arc, protective):** stand up any CI harness, then promote
  ...
  payoff is live; a runtime/type fence in `serval-scripted-dom` to make the
...
- **Resist (premature):** webgl-essl integration ahead of a canvas consumer
  ...
  (serval chrome on a host framework, two of everything); go to architecture 3,
...
- Tests run this pass: `serval-layout` box_tree 9/9; `paint_list_render` 10/10;
...
larger-frame done as structured findings; webgl and serval-as-host grounded from
