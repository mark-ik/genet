...
| [components/paint/interop/](../components/paint/interop/) | Direction-neutral foundation extracted from `wgpu-native-texture-interop` patterns (no WNTI dep): `HostWgpuContext`, `InteropBackend`, `SyncMechanism`, `InteropError`, `Dx12FenceSynchronizer` (Windows-only). Per-platform synchronizers live as inherent impls — no import-direction-coupled trait. |
