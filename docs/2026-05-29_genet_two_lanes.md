# genet two development lanes (2026-05-29)

Operationalizes the [holistic audit](./2026-05-29_genet_holistic_audit.md):
...
**Terminology note.** "Lane" is already used in genet for **Hekate
engine-routing** lanes (Nematic / Genet / Scrying;
...
a **different axis**: parallel *workstreams inside Genet*, not routing
...
```text
   ...
   (genet runs the real web)        (genet hosts the app, chrome as CSS)
   WPT / JS / DOM breadth            xilem-serval / native authoring
            \                                      /
             \                                    /
              \------------- shared spine -------/
        one DOM (genet-scripted-dom + LayoutDomMut)
...
## Lane C — Content (genet runs the real web)
...
- **Parsing / input formats** — what genet can turn into a `LayoutDom`.
  ...
  to drive genet-static-dom's existing `StaticTreeSink` with `xml5ever::parse_document`
...
*library* (`hx-*` attributes + `fetch` + DOM swapping), so "genet runs htmx
...
## Lane H — Host (genet hosts the app)

**Mission.** Author Mere's chrome (and eventually content shells) as documents
in genet, via the `xilem_serval` reactive backend: Rust app state to a view
tree to DOM mutations, genet the sole engine. Architecture 3 from the host
doc.

**Current state.** Implemented through Stage 2 + on-screen demo: `xilem_core`
backend over `ScriptedDom`, `GenetAppRunner`, faithful event dispatch
(capture/target/bubble), keyboard + focus, form-control start, caret editing.
`pelt-live-counter` runs a reactive counter on screen with genet as the only
engine.
...
- **Overlays / popups** — dropdowns (`<select>`), context menus, tooltips,
  modals/dialogs. A document engine has no layer above the flow; genet-as-host
...
through `xilem_serval` with genet as the sole engine, input and a11y working.
Owner doc:
[genet-as-host](./2026-05-27_genet_as_host_xilem_serval_plan.md).
...
(`genet-scripted-dom`, `genet-layout`, `script-engine-api`,
...
"architecture 2" (genet chrome on a foreign host framework, two of
