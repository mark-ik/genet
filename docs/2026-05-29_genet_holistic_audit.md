# genet holistic audit (2026-05-29)

> **Superseded by [2026-06-02_genet_holistic_audit.md](./2026-06-02_genet_holistic_audit.md).**
...
genet is converging two arcs onto one substrate. Arc A is web conformance
(run the real web: testharness, DOM, eventually fullweb). Arc B is
genet-as-host (genet's own DOM as a native Xilem-driven application UI,
...
Both ride one DOM (`genet-scripted-dom` + `LayoutDomMut`), one event model
...
- **Layout: strong.** Block, inline, floats, replaced `<img>`, the planes
  ...
  and lay out: `genet-wpt reftest css/CSS2/floats` pixel-compares real inline
...
- **Interaction: hit-test + box-model done; focus / affordance / selection /
  activation queries stubbed** (`genet_lane.rs`, probe v1).
...
- **Profile ladder** (static-html / interactive-html / scripted / fullweb as
  ...
  architecture).
- **Hekate lanes** (Nematic / Genet / Scrying):
  [2026-05-17_hekate_lanes_observables.md](./2026-05-17_hekate_lanes_observables.md).
  Genet has internal tiering; Hekate routes by engine, Genet picks the tier.
...
- **Live: two masters for the DOM.** `genet-scripted-dom` serves both
...
- **Tier gates are manual.** Nothing in CI fails if `genet-static-html`
...
- **Resist (premature):** the weval/wasm-JS arc (prototype-gated, thin bus
  ...
  GUI wobbles, do not reach for "architecture 2" (genet chrome on top of a
...
- WPT (Boa): html/dom 35366/49556, dom/nodes 832/4739, dom/lists 100/189,
  dom/traversal 32/48 (run via `genet-wpt testharness <subset>`).
- Reftest (real CSS): `genet-wpt reftest css/CSS2/floats` 7 passing pixel
