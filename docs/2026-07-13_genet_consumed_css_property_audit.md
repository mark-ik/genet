# Genet consumed CSS property audit (current Genet paths)

**Date:** 2026-07-13  
**Status:** landed against Genet `6b955ff96ed` and Stylo `eec60c2464`

Genet is the engine product formerly called Genet. Current repository, crate,
and source paths retain `genet` compatibility names during the migration.
...
current Genet layout, paint, CSSOM, and animation path actually consume?

## Result

The current full Genet contract is **126 longhands across 16 Stylo style
...
| Direct Genet render/text/box reads | 73 | `components/genet-layout`, especially `paint_emit.rs`, `box_tree.rs`, and `construct/` |
| `getComputedStyle` exposure | 30 | `components/genet-layout/computed_query.rs` |
| Animation and transition controls | 13 | Genet's Stylo animation path and `style/servo/animation.rs` |
...
- `R`: direct Genet render, text, box, or construction read
- `C`: `getComputedStyle`
...
feature swap of the current `genet-layout` crate. The live crate also relies
on:
...
- 257 `style::` references spread across 24 `genet-layout` source files.
...
1. **Full `genet-layout` swap parity:** the new engine starts with this
...
These 126 longhands are the hard keep-set for the current Genet product path.
...
- active `stylo_taffy` features in `components/genet-layout/Cargo.toml`;
- direct computed-style reads under `components/genet-layout`;
