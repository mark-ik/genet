...
**Source:** [github.com/gterzian/formal-web](https://github.com/gterzian/formal-web), a Rust web engine by Gregory Terzian (ex-Servo event-loop maintainer). Studied via a 6-agent fan-out reading the repo + Terzian's writing, mapped against genet's actual seams (file:line).
**Why it matters:** it is Boa-based (genet's own wasm JS backend), spec-faithful by construction, and its open problems are exactly genet's audit-named §6 async gaps (`2026-06-24_grand_audit.md`). Terzian fixed the same event-loop bugs in Servo that genet's message-passing constellation is structurally prone to. So this is a concrete playbook, not an analogy.
...
Terzian used this toolkit to catch two real Servo event-loop bugs, and genet's "scenes travel as messages" constellation is especially prone to both:
...
5. **Agent / agent-cluster as the concurrency boundary (the concept, not the process).** `Agent { id, can_block, event_loop_id }` / `AgentCluster` is spec-level data. Adopt agent-cluster as genet's SharedArrayBuffer/Atomics boundary; bind `EventLoopId` to an armillary actor (native) or a Web Worker (wasm) instead of an OS process. `can_block` (workers may block, windows may not) is directly the Web-Worker-as-agent plan. **Genet seam:** `repos/mere/crates/armillary/src/actor.rs:107`.
6. **Hand-written runtime WebIDL bindings registry (no codegen, no bootstrap-JS string).** A `WebIdlInterface` trait registers `OperationDef`/`AttributeDef`/`ConstantDef`; `register_interface_spec::<T>(ctx)` materializes the prototype and installs members as Boa `NativeFunction`s; platform objects are `#[derive(Trace, Finalize, JsData)]` Rust structs in `JsObject`s with `downcast_ref::<T>()` as the runtime type check; exotic objects use the public `JsProxyBuilder` (never Boa `pub(crate)` internals); async APIs keep promise resolvers in a side table keyed by `request_id`. **Genet seam:** replaces the ~900-line JS bootstrap string; the side table maps onto `new_host_promise`/`settle_host_promise` (`components/script-engine-api/lib.rs:201`); because algorithm bodies hold no `JsValue`, it supports the dual Boa/Nova goal. The public-API-only rule respects genet's no-fork-deps doctrine.
7. **TLA+ trace validation itself.** Architecture-agnostic (needs an event log + a model). genet's single-process model makes it *easier* than formal-web (one in-process channel, one counter clock, no cross-process monitor or channel-closure quiescence dance). The five WHATWG task boundaries are already named Rust calls to tap. Months-shaped CI investment, so do it for one protocol, not the engine.
...
formal-web's multiprocess-per-agent OS architecture is the opposite of genet's single-process in-process-actor constellation + wasm Web-Worker isolation. Do not copy:

- The `ipc/` OS-process bootstrap (`IpcOneShotServer`, Mach ports/fds, XPC backend, child-process handles). genet uses in-memory messages (native) or postMessage (wasm).
- The cross-process trace monitor (`TraceSender` clone-drop as global-quiescence detection, per-process `producer` tagging merged over IPC). genet needs one in-process mpsc + an explicit "trace complete" signal.
- `EventLoopId -> OS process` binding and multiprocess WindowProxy window-swapping.
- `wasmtime` for in-page wasm: genet's browser target bars a nested JIT and defers to the host `WebAssembly` object. Only the off-thread-compile + side-table-resolver + microtask-checkpoint *structure* transfers.
