...
`mark-ik/servo-media-features` at `28f4c49`; serval-layout suite green (265).
**Scope:** bring stylo's Servo-mode media-feature set up to the standard
`@media` surface, in the `mark-ik/stylo` fork (branch
`mark-ik/servo-media-features`), plus the serval-side `Device` plumbing. Author
CSS media queries, evaluated correctly. Cross-project: every serval embedder
...
serval's cascade, and it is spec-conformance work every project rendering
through serval needs regardless. M0 (`prefers-reduced-motion`) proved the fork
pipeline; this plan carries it to parity.

## Current state (2026-07-06)

Servo table + the fork's M0 add = 6 upstream + `prefers-reduced-motion`. The
fork/serval wiring pattern per feature, established at M0:
...
5. serval: thread it through `cascade.rs::make_device_with_prefs`; a live
...
| `scripting` | `Scripting` (none/initial-only/enabled) | whether serval's JS runtime is live |
...
- **stylo fork:** one `MediaEnvironment` value struct (in `queries::values` or
  ...
  `prefers_reduced_motion()` getters kept as delegating shims so serval's
  current setters keep compiling.
- **serval:** one `set_stylist_media_env(stylist, lock, viewport, quirks, env)`
...
- **M1 — Tier A + B (geometry + constants).** *Landed 2026-07-06* (fork commit
  ...
  viewport/dpr or fixed screen constants; no new Device state, no serval device
  ...
  portrait one. serval-layout suite green (258).
- **M2 — MediaEnvironment consolidation.** *Landed 2026-07-06* (fork commit
  ...
  read-modify-write shims. serval: `make_device_with_prefs` takes a
  ...
  rule still applies. serval-layout suite green (259). Each later phase now adds
...
- **M5 — Tier E (app/engine state).** *Landed 2026-07-06* (fork commit
  ...
  the default (browser + enabled). serval-layout suite green (263).
  - **Remaining wiring:** `scripting` defaults to `enabled`; the host should set
    it from whether its script runtime is actually live (serval's static-layout
    path is `scripting: none`). That auto-wiring is a serval-host concern, not a
...
- **Cross-cutting — WPT.** *Landed 2026-07-06.* Wired the `css/mediaqueries`
  ...
  **44 passed, 4 failed, 45 skipped** (of 93). The 44 include every
  ...
  - **Why reftests, not testharness:** the WPT `css/mediaqueries` *testharness*
    ...
    …) drive `window.matchMedia()`, a JS API serval does not expose — the media
    ...
    script-free.
  - **Harness change:** extended serval-wpt's JSON expectations mechanism
    ...
    `ports/serval-wpt/expectations/reftest/css_mediaqueries_boa.json` (93 tests).
...
- **matchMedia — *landed.*** `window.matchMedia(query)` end to end, **no fork
  ...
  snapshot for now). serval-scripted wires a `MediaQueryBridge` over the retained
  frame, alongside the computed-style bridge. Guards:
  `evaluate_media_query_matches_and_serializes` (serval-layout) and
  `match_media_evaluates_against_the_frame_on_boa`/`_on_nova` (serval-scripted,
  end to end).
  ...
  - **WPT testharness measurement (2026-07-08):** wired a `MediaQueryHandler`
    over a default 800x600 device into the serval-wpt runner
    (`serval_layout::MediaQueryEvaluator`), so the `css/mediaqueries`
