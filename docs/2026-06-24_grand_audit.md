...
- **Engine correctness (CSS reftests + DOM testharness).** CSS remains the long pole by raw volume (the local checkout is 46,907 CSS files vs 11,516 html / 629 dom / 547 fetch). CSS snapshot numbers from the original audit still frame the major buckets: floats 42/197, normal-flow 462/1044, css-backgrounds 334/1325, css-images 172/713, css-writing-modes 219/1829, css-multicol 103/923, css-tables 61/402. DOM/testharness numbers in this audit are superseded by the manifest-backed WPT baselines: full `dom` on Boa is pinned at 88 all-pass, 365 with-failures, 72 errored, 84 no-results, 51 skipped, with 2122/6671 subtests passing; focused `dom/nodes` is pinned at 57 all-pass, 205 with-failures, 12 errored, 14 no-results, 42 skipped, with 1655/5365 subtests passing. *(Corrected 2026-07-09: the full-`dom` figures went stale when `dom_boa.json` was rebased 2026-07-05. A live release run now reports **91 all-pass, 363 with-failures, 71 errored, 84 no-results, 51 skipped of 660 files, subtests 2230/6654**, at `unexpected=0` against the checked baseline. The errored + no-results set is triaged in the harness-exactness plan's H6.)*
- **Harness runnability (gates whole directories before any engine work counts).** This is the binding constraint. Updated 2026-06-30: the runner now uses MANIFEST.json-backed discovery by default, with the old hand-rolled directory walk demoted to `--walk-discovery`; `testharness` has JSON expectation write/check mechanics, checked Boa baselines for full `dom`, focused `dom/abort`, focused `dom/nodes`, and `html/webappapis/timers`, plus a local/CI guard that enforces `unexpected=0`. A fresh Runtime still re-evals the 5,207-line testharness.js every test.

**Directories that do not run, or run un-scored:** CSP is effectively 0 (greenfield in the live lane; the only `content-security-policy` refs are legacy Servo `shared/` crates, not wired in); websockets/ and h3 exist in netfetcher but are not wired into the runner; testharness XHTML/.xht files are skipped while XML reftest handling is separate; iframe/second-realm tests are unrunnable, walling off chunks of fetch/ and most of html/; fetch/ runs through an off-by-default server-mode/netfetch lane with `--spawn-server`, not the default disk-mode guard.
...
1. **MANIFEST.json reader (HARNESS, large).** Updated 2026-06-29: this lever is implemented for the runner-owned hostable window subset. Normal commands discover from MANIFEST.json by default; generated `.any.html` variants retain their WPT URL while resolving from the backing `.any.js`; reftest refs/fuzzy come from the manifest. `--walk-discovery` keeps the old heuristic path only as a diagnostic fallback. *(Plan: WPT harness exactness.)*
2. **object-fit / object-position (ENGINE/CSS, medium).** The real css-images lever, named by none of the candidates: **123 of 226** css-images fails, and replaced-element fit/position is reused by `<img>/<video>/<canvas>` sizing well beyond css-images. Unimplemented (`components/genet-layout/construct.rs:113` is only a comment). ~100 reftests plus spillover. *(Plan: CSS conformance, extended.)*
3. **Snapshot-clone Runtime pool (HARNESS, medium).** The bench probe proved the dominant per-test cost is the testharness.js eval, that naive reuse leaks the `tests` singleton, and prescribed a post-harness-eval snapshot per test (`harness.rs:393-414`). Updated 2026-06-30: this is built as a Nova-owned `snapshot_clone` capability plus `ScriptEngineSnapshot`, `Runtime::snapshot_clone`, and `NovaHarnessTemplate` in `genet-wpt`. The remaining blocker is practical stability: broad release-mode `dom` on Nova still stack-overflows before a final aggregate, although smaller slices complete and `dom/nodes` shows the intended speedup. *(Plan: WPT harness exactness.)*
...
The transform-motion relayout fear is retired (genet spike 2026-06-01, restyle stays RepaintOnly), so the budget is scene rebuilds, allocations, and redraw cadence, not layout. Wire per-pane timers first; the C0 profile harness exists (`render.rs:1988-1993`) but lacks per-pane granularity.
...
It is the third xilem_core backend (beside Masonry and xilem_web): "xilem_web, but native, with genet as the engine." It diffs a Xilem view tree into the shared `ScriptedDom` and does no layout/paint/a11y itself. Well past probe stage (Stages 0-7 done, ~6,801 LOC, 54 tests). The clean win: it reuses xilem_core's message cycle unforked, with a uniform `GenetElement = NodeId` element (no AnyNode/downcast).
...
## 8. A vision of genet's architecture

genet is not "a browser engine" in the Servo sense. It is a deliberately decomposed web platform: planes bolted together by narrow engine-neutral seams, with capability dialed by a profile ladder and the web-engine chosen by a multiplexer.
...
1. **`paint_list_api` as the universal display-list contract.** Everything downstream rests on genet producing `PaintCmd` and netrender being one consumer. Current cost: the lowering layer under-delivers genet's real fidelity (`paint_list_render` `ScenePathStroke` is `{color,width}` so cap/join/dash drop; double/groove borders fall to square; several blend modes fall to Normal). Pure translation work, tracked as the netrender lowering tail in the CSS conformance plan.
...
- `2026-06-24_capability_activation_plan.md` — §5 sidequests: XPath `Document.evaluate`, genet-extract -> Inspector + crawler, live WebGL `<canvas>` via external-texture, genet CI + witness guard.
