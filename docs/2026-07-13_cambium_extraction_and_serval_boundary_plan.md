...
2. `components/xilem-serval` is an application toolkit with controls, editors,
   ...
   views. Its only natural relation to Serval is that it targets Serval's DOM.
...
4. Applications currently pin Serval `main` for the GUI crates. UI updates are
   therefore coupled to CSS-engine realignment and browser-engine churn even
   when their UI dependency did not change.
5. The reverse dependencies from Serval into `serval-xilem` are small and
...
- Serval source: `6b955ff96ed8b2912d04f7a36a85a36b401bb780`.
...
2. Import the Serval histories for `components/xilem-core`,
...
The Serval-authored Cambium backend retains its MPL headers. Meristem retains
...
metadata, and provenance ledger are in place. The four Serval core commits, 113
...
- `git log --follow` reaches the relevant Serval history for Cambium and
  Sprigging; Meristem's two-parent history graft reaches both filtered Xilem and
  Serval lineage with `git log --full-history`.
- The workspace contains no Masonry or `xilem_web` packages.
- `cargo metadata --no-deps` succeeds without a Serval checkout path override.
- No package has been published and no remote state has changed.

### C1 - Make `meristem` the honest core fork

Start from the live Serval copy, because it contains the move-preservation API
...
- No Meristem module depends on Serval, Sprigging, winit, or a renderer.

### C2 - Move the Serval backend and Sprigging
...
test against published Serval seams, and Sprigging's default feature tree has
...
- Update module documentation: Serval is the engine; Sprigging is the custom-leaf
...
- The component-catalog examples render using Serval as the sole layout/paint/
  input/accessibility engine.
- Cambium depends on versioned or git-addressable Serval seam crates, not paths
  back into the source checkout.

### C3 - Break every Serval -> Cambium edge
...
- `cambium-winit` depends on winit and Cambium. Applications compose it beside
  `serval-winit-host`; it does not wrap or depend on that presentation crate.
- Keep `RenderCore`, `WindowSurface`, `SurfaceHost`, wheel normalization, and the
  generic AccessKit bridge in Serval.

Done when `serval-winit-host` has no Cambium dependency and its tests still
...
- Neutralize the engine's current Chisel-specific DOM vocabulary. Rename the
  ...
  Serval. Cambium and Sprigging stamp and consume that protocol. Keep the old tag as
...
- Move `RenderedLeaves` adapters and `*_with_leaves` convenience assembly into
  Cambium, or make the Serval entry points fully generic over the neutral source.
- Move `serval-render` tests that construct a `ServalAppRunner` into Cambium
  integration tests. Serval-render tests should construct DOMs directly.

Done when `serval-render` and `serval-layout` have no normal or dev dependency
...
- If `serval-documents` retains a smolweb feature, make it consume that
  engine-native pipeline. The Cambium-native DOM view is a downstream option,
  not the definition of a Serval document session.

Done when Nematic and `serval-documents` build without Cambium, while
...
1. Pelt, as the reference integration surface inside the Serval checkout.
...
Strophe:   cargo test -p strophe-serval
...
### C5 - Publish or pin, then remove the Serval copies
...
- Serval's workspace passes without the three moved directories.
- Cambium's Serval compatibility table names the exact supported Serval package
...
- Remove the moved workspace members and path dependencies from Serval.
...
- Keep historical implementation docs, but label their old crate homes as
  historical.
- Update application docs to say Cambium authors the UI and Serval renders it.

Done when:

- `rg "serval-xilem|serval-xilem-core|serval-chisel|xilem_serval"` finds only
  historical/supersession text in Serval and migrated application repositories.
- No Serval engine package has Cambium in its normal or dev dependency closure.
- A clean Cambium checkout can run its tests against the declared Serval source.
- A clean Serval checkout can run its engine tests without a Cambium checkout.

## Verification walls

### Dependency direction

For every Serval engine package, `cargo tree` must show no Cambium package.
Pelt may depend on Cambium because it is a reference host, not an engine crate.

Minimum audit set:

```text
cargo tree -p serval-layout
cargo tree -p serval-render
cargo tree -p serval-winit-host
cargo tree -p serval-documents
...
**Risk:** a convenience dependency leaves Serval depending on Cambium while
Cambium depends on Serval.

**Stop rule:** do not delete the Serval copies or publish Cambium until the C3
...
window presentation remain Serval responsibilities. Sprigging contributes leaf
...
- Preserving `xilem_web` as a Cambium backend. Cambium is Serval-native.
- Moving Inker, document-canvas, Nematic's engines, or Serval's renderer into
...
7. Clean checkouts of Serval and Cambium verify independently against their
