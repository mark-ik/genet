# genet-layout: infrastructure scope (post emit-feature push)
...
`GenetLaneView` interaction state read-back; selection-range geometry.
...
~727). So the *matching* half is done. `GenetLaneView` also now has partial
...
- Extend the current `GenetLaneView::with_interaction` path, or replace it with
...
**Touch-points:** `cascade.rs` (state population + invalidation), `genet_lane.rs`
...
**Touch-points:** `layout_dom_api` (trait method + default), `genet-static-dom`
