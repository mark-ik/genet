# Content Inspection — serval's devtools substrate (scope)
...
block-structure summary — **for the non-serval `EngineDocument` path**. For
serval-rendered HTML it prints *"HTML rendered through Serval; no EngineDocument
diagnostics"*. So serval, the engine, renders a page but hands its own Inspector
nothing to inspect: **serval-rendered content is second-class in mere's content
devtools.** Closing that is the point.

## The capability

A serval-level **content-introspection surface**: `DOM -> ContentReport`
...
layer). serval already renders the page into a `ScriptedDom`/`StaticDocument` and
already emits the accesskit a11y tree (`serval-render/src/a11y.rs`); this packages
...
Same shape as render + present. **serval produces** the introspection surface;
...
serval HTML" line goes away). The surface is engine-level, not a pelt feature.

## Slices

1. **Structure slice (this pass)** — `content_report(dom)` = title + outline +
   links + headings, in serval-render; re-point pelt's driven tests off the tile
