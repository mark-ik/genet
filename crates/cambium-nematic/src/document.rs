...
//! Smolweb documents as native Genet views (the `document` feature).
//!
//! Errand parses host-provided bytes into a per-format AST, and a Cambium view renders it into a
...
//! the windowed viewer that presents it is an application-level integration.
...
use cambium::{DomHandle, PointerClick, ServalAppRunner};
...
impl cambium::Action for Navigate {}
...
/// A retained smolweb document: a Genet `ScriptedDom` built from a native smolweb
